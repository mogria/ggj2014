...
{
	mMoveSpeed = 50;
	mSceneMgr = OgreFramework::getSingletonPtr()->m_pRoot->createSceneManager(ST_GENERIC, "GameSceneMgr");

	mWorld = new OgreBulletDynamics::DynamicsWorld(mSceneMgr, 
		AxisAlignedBox(Ogre::Vector3(-10000, -10000, -10000), Ogre::Vector3(10000, 10000, 10000)), Ogre::Vector3(0,-9.81,0));
...
void GameState::createScene()
{
    ...
    m_pSceneMgr->getEntity("Cube01")->setVisible(false);
    m_pSceneMgr->getEntity("Cube02")->setVisible(false);
    m_pSceneMgr->getEntity("Cube03")->setVisible(false);
...
void GameState::update(double timeSinceLastFrame)
{
	...
	mWorld->stepSimulation(timeSinceLastFrame);   // update Bullet Physics animation
...
void GameState::createBox()
{
	Vector3 size = Vector3::ZERO;   // size of the box
    // starting position of the box
	Vector3 position = Ogre::Vector3(0,40,0);
